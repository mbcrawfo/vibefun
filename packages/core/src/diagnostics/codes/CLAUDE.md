...
- **Each phase file (or phase folder's `index.ts`) ends with a `registerXxxCodes()` function that must be called from `./index.ts`.** New phase files/folders must be wired there or their codes won't be registered, and `throwDiagnostic` will throw a "code not found" error.
