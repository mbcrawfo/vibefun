        ...
        it("property: every node in a parsed module carries a Location with file/line/column/offset", () => {
            fc.assert(
                fc.property(declArb({ depth: 2 }), (d) => {
                    ...
                    // ListElement is the one discriminated wrapper without a `loc`
                    // field (`{ kind: "Element"|"Spread", expr }`). Listing it as an
                    // explicit exemption catches a regression that drops `loc` from
                    // a real AST node — using "loc" in obj would silently pass.
                    // RecordField.Spread shares the "Spread" tag but does carry loc;
                    // it's not generated by the current arbitraries, so this trade
                    // is acceptable.
