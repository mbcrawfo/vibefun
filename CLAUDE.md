...
5. **Comprehensive Testing**: All code changes must include coverage at every layer that the change touches. Default to adding tests at each of these layers unless a layer is clearly inapplicable:
   ...
   - **Let-binding matrix sync** — when adding or changing a `let` / `let rec` form (parser admits a new shape, desugarer routes differently, typechecker grows a new path), add the form to `tests/spec-validation/sections/07b-let-binding-matrix.ts` so every soundness scenario runs through every form. Cross-path divergence then surfaces as a single flat-list cell instead of a ninth review round.
