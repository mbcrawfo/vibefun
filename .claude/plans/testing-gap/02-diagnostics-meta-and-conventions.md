...
4. **`tests/e2e/docs-errors-sync.test.ts`** (new) — Layer: E.
   - F-20: spawn `pnpm docs:errors` in a temp clone of `docs/errors/`, then `git diff` the output against committed files. Fail if any change. (If a CI step already does this, prefer extending it; otherwise add the e2e.)
